...
O² 是主实现。它的默认语义是 box OCD、六变量 ADR、D-FINE decoder-wide geometry union matching，以及已公开 O² 检测器实际使用的 L2 four-corner Chamfer assignment。angle 配置是独立的标量角度架构对照。

两个方法入口是并列专门化，只共同继承内部
`configs/base/dfine_obb.yml` 的 D-FINE-OBB-M 结构与优化契约；O² 不继承
direct-angle 的 loss、matching 或 denoising 语义。
...
| Chamfer | 四角点双向平均 Euclidean distance | 已公开 O²-RTDETR `ChamferCost` 的可执行语义 |
| 几何监督匹配 | decoder/pre/encoder assignment union | D-FINE 原语义 |
| OCD crowded policy | `released_dynamic` | 已公开 O² 去噪逻辑 |
| 分类与 FGL 质量 | aligned XYWH IoU | D-FINE FGL 与公开 O² 的 `hbox_iou` 语义 |
...
编码为同一个 ADR 几何结果：top/right 是图像坐标中的全局极值顶点，不是随局部 `(w,h)` 命名交换的顶点。因此提问中“交换参数后 top/right 必然交换”的前提不成立，ADR 不会仅因这组等价参数化生成两个 target。
