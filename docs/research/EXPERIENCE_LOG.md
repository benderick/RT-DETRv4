...
## EXP-2026-012：对称性正确的去噪分布，不必然带来更有效的训练信号

- **命题**：DETR 去噪查询的扰动不应依赖具有表示歧义的坐标幅度，而应在旋转框的
  等价轨道上保持一致，并用任务风险壳层定义正负对比查询；这样既满足输出对称性，
  也应比普通 box noise 更快或更好地训练 O²。
- **证据**：UAV-ROD、seed 42、稳定 O² 与 Orbit-DN 受控比较 24 epoch；Stage 0 和
  原型门禁分别保存在 `logs/research/orbit_dn/uav_rod_stage0/` 与
  `logs/research/orbit_dn/uav_rod_prototype_gate.json`。
- **机制结果**：正、负风险壳层占用率分别为 `0.998355/0.999825`，风险顺序违反率为
  `0`，训练保持有限值，median step 开销为基线的 `1.071916`。这证明轨道等变扰动与
  风险壳层可以一致实现，失败不能归因于机制没有真正运行。
- **否决原因**：epoch 24 的 mAP50-95 只从 `0.857807` 提升到 `0.859495`
  （`+0.001688`），低于预注册的 `+0.005`；原型也到 epoch 24 才达到基线 epoch-24
  质量，没有满足最迟 epoch 21 的收敛加速门槛。几何上更规范的 corruption kernel
  没有转化为足够强的优化优势。
- **以后规则**：对称性一致只是去噪机制的必要性质，不是有效性的充分条件。再研究新的
  DN 分布前，必须先证明现有 box noise 在真实训练中造成可测的等价类风险错序、梯度
  冲突或样本效率瓶颈；仅有漂亮的不变性与壳层占用诊断，不授权进入完整训练。
